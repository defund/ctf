use fancy_garbling::{
    errors::TwopacError, WireLabel, AllWire, ArithmeticWire, Fancy,
...
use crate::garble_evaluator::Evaluator as Ev;
...
impl<C: AbstractChannel, RNG> Evaluator<C, RNG, Receiver, WireMod2> {
    pub fn and_curious(&mut self, x: &WireMod2, y: &WireMod2) -> Result<(WireMod2, Option<(bool, bool)>), TwopacError> {
        self.evaluator.and_curious(x, y, &self.ot.delta).map_err(TwopacError::from)
    }
}
