...
use fancy_garbling::{
    // check_binary,
    errors::{EvaluatorError, FancyError, TwopacError},
    hash_wires,
    util::{output_tweak, tweak, tweak2},
    AllWire, ArithmeticWire, Fancy, FancyArithmetic, FancyBinary, FancyReveal, WireLabel, WireMod2,
};
...
impl<C: AbstractChannel, Wire: WireLabel> Evaluator<C, Wire> {
    ...
    pub fn and_curious(&mut self, A: &WireMod2, B: &WireMod2, delta: &WireMod2) -> std::io::Result<(WireMod2, Option<(bool, bool)>)> {
        let gate0 = self.channel.read_block()?;
        let gate1 = self.channel.read_block()?;

        let c0 = self.evaluate_and_gate(&A.plus(delta), &B.plus(delta), &gate0, &gate1);
        self.current_gate -= 1;
        let c1 = self.evaluate_and_gate(&A.plus(delta), B, &gate0, &gate1);
        self.current_gate -= 1;
        let c2 = self.evaluate_and_gate(A, &B.plus(delta), &gate0, &gate1);
        self.current_gate -= 1;
        let c3 = self.evaluate_and_gate(A, B, &gate0, &gate1);
